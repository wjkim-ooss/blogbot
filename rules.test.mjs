...
test("② 모을 때 업종이 섞이면 많은 쪽만 남긴다", () => {
  const list = [
    { keyword: "마곡 피부관리", 업종: "피부", posts: [
      { title: "마곡 피부관리 기준", text: "관리 이야기", chars: 2000, images: 10 },
      { title: "마곡 피부관리 후기", text: "관리 이야기", chars: 2000, images: 10 },
      { title: "여드름", text: "마곡 피부관리를 받았습니다", chars: 2000, images: 10 },
    ] },
    { keyword: "분당 네일아트", 업종: "네일", posts: [
      { title: "네일 이야기", text: "마곡 피부관리도 같이 받아요", chars: 2000, images: 10 },
      { title: "젤네일", text: "마곡 피부관리 후 네일까지", chars: 2000, images: 10 },
    ] },
  ];
  const r = 본문에서찾기(list, "마곡 피부관리", { 최소: 3 });
  assert.deepEqual(r.출처, ["마곡 피부관리"], "제목에 키워드가 든 쪽 업종만 남아야 한다");
  assert.equal(r.posts.length, 3);
});

test("② 가르면 최소에 못 미치거나 업종 표시가 없으면 가르지 않는다", () => {
  const 섞임 = [
    { keyword: "모공", 업종: "피부", posts: 글("모공", "블랙헤드 이야기", 2) },
    { keyword: "네일", 업종: "네일", posts: 글("네일", "블랙헤드 이야기", 2) },
  ];
  assert.equal(본문에서찾기(섞임, "블랙헤드", { 최소: 3 }).posts.length, 4, "가르면 2편뿐이라 그냥 둔다");

  const 표시없음 = [
    { keyword: "모공", posts: 글("모공", "블랙헤드 이야기", 3) },
    { keyword: "네일", 업종: "네일", posts: 글("네일", "블랙헤드 이야기", 3) },
  ];
  assert.equal(본문에서찾기(표시없음, "블랙헤드", { 최소: 3 }).posts.length, 6, "표시가 없으면 판정을 지어내지 않는다");
});
...
// 2026-09-14 우진: "문장이 긴 게 티가 나면 절대 안 돼" — 권장에서 불합격으로 올렸다 (config.문장길이.불합격).
test("문장이 길면 불합격으로 걸어 다시 쓰게 하고, 짧으면 조용하다", () => {
  const 긴 = 재기(줄글(긴줄), { 원장값 });
  const 짧 = 재기(줄글(짧은줄), { 원장값 });
  assert.ok(긴.issues.some((s) => s.includes("긴 문장") || s.includes("문장이 길다")), `긴 글이 불합격이 아니다: ${긴.issues.join(" | ")}`);
  assert.ok(!짧.issues.some((s) => s.includes("긴 문장") || s.includes("문장이 길다")));
  // 스위치를 끄면 예전처럼 권장으로 내려간다
  const 옛 = 평가(줄글(긴줄), { keyword: "여드름", config: { ...CONFIG, 문장길이: { ...CONFIG.문장길이, 불합격: false } }, 말투: "요약", 원장값 });
...
test("한 문장을 여러 줄로 나눠 쓰거나 쉼표로 끊으면 긴 문장으로 세지 않는다", () => {
  // 상한이 20자라 세 줄로 나눈다 — 각 줄이 공백 빼고 20자 안
  const 두줄 = "여드름이 반복되는 이유는\n제품이 나빠서가 아니라\n세안 후 방치하는 시간과\n덧바르는 습관이 겹치기 때문입니다.";
  const 쉼표 = "여드름이 반복되는 이유는, 제품이 나빠서가 아니라, 세안 후 방치하는 시간과, 덧바르는 습관이 겹치기 때문입니다.";
  assert.equal(재기(`제목: 여드름 피부관리\n\n${(쉼표 + "\n").repeat(14)}`, { 원장값 }).문장길이.긴것.length, 0, "쉼표로 끊은 토막은 따로 센다");
...
// 2026-09-15 네 번째 초안: 35문단 중 8개가 "돈을 아껴야 합니다"류로 끝났고, 175문장이 전부 11자 안팎이었다.
test("문단 끝 당부가 허용을 넘게 되풀이되면 짚고, 하한에 못 미치는 토막이 많아도 짚는다", () => {
  const 당부문단 = "함량이 낮습니다. 색만 보라색입니다. 성분표를 확인하세요.\n\n";
  const 사실문단 = "함량이 낮습니다. 색만 보라색입니다. 뒤쪽 순서에 적혀 있습니다.\n\n";
  const a = 재기(`제목: 여드름 피부관리\n\n${당부문단.repeat(6)}`, { 원장값 });
  const b = 재기(`제목: 여드름 피부관리\n\n${사실문단.repeat(6)}`, { 원장값 });
  assert.equal(a.당부문단.length, 6);
...
// 2026-09-15: 정보 유형 초안에 "직접 레티놀 0.3%를 12주 적용하며 관찰… 눈으로 확인했습니다"가 나왔다.
// '직접 해본 것'을 준 적이 없는 사람이 직접 본 것처럼 쓰면 지어낸 경험이다 — 발행 전에 지우게 짚는다.
test("정보 전달자가 직접 해본 것을 안 줬는데 직접 관찰한 것처럼 쓰면 짚는다", () => {
  const 글 = 구체글() + "\n직접 발라 보며 12주 동안 관찰해 본 자료가 있습니다. 8주차부터 안정되는 것을 눈으로 확인했습니다.";
  const 없이 = 재기(글, { 원장값: 정보값, 유형: "정보" });
  assert.equal(없이.지어낸경험.length, 2, JSON.stringify(없이.지어낸경험));
  assert.ok(없이.advice.some((s) => s.includes("직접 해본 것처럼")), 없이.advice.join(" | "));
  const 주고 = 재기(글, { 원장값: { ...정보값, 직접해본것: "레티놀 0.3% 12주 사용" }, 유형: "정보" });
  assert.equal(주고.지어낸경험.length, 0, "직접 해본 것을 줬으면 직접 봤다고 써도 된다");
  const 원장 = 재기(글, { 원장값, 유형: "원장" });
  assert.equal(원장.지어낸경험.length, 0, "원장은 샵 이야기가 곧 직접 경험이다");
  const 열람 = 재기(글, { 원장값: null, 유형: "정보" });
  assert.equal(열람.지어낸경험.length, 0, "남의 초안 열람 때는 판단하지 않는다");
});

// 2026-09-15: 아줄렌 초안에 "PMID 12345678"이 나왔다 — 자리표시 숫자는 지어낸 근거다. 근거 없음과 같은 불합격.
test("자리표시 PMID(12345678·같은 숫자 반복)는 지어낸 근거로 본다", () => {
  const 글 = 구체글() + "\n레티놀은 여드름을 감소시킵니다(PMID 12345678). 살리실산도 각질을 줄입니다(PMID 11111111).";
  const v = 재기(글, { 원장값 });
  assert.ok(v.issues.some((s) => s.includes("지어낸 PMID")), v.issues.join(" | "));
  assert.ok(v.가짜PMID.includes("12345678") && v.가짜PMID.includes("11111111"), JSON.stringify(v.가짜PMID));
  const 진짜 = 재기(구체글() + "\n레티놀은 여드름을 감소시킵니다(PMID 34980969).", { 원장값 });
  assert.ok(!진짜.issues.some((s) => s.includes("지어낸 PMID")));
});
...
// 피부과 원장도 "원장입니다"라고 쓴다 — 환자·진료실·처방·시술 후 같은 병원 말로 갈라야 한다.
// (원장 글 보관함 첫 수집에 청담 피부과·"논문읽는 조원장" 글 28편이 원장으로 섞여 들어왔다)
test("피부과 원장 글은 원장이 아니라 병원이다", () => {
  const 의사 = { title: "화농성 여드름, 먹는 약이 1순위인 이유", text: "안녕하세요 조원장입니다. 진료실에서 환자분들을 보면 처방을 두려워하십니다. 시술 후 관리도 중요합니다." };
  const 원장 = { title: "턱 밑 모낭염 관리 기록", text: "안녕하세요 원장입니다. 오늘 오신 고객님께 압출 대신 진정부터 해 드렸습니다. 저희 샵은 첫 방문에 압출부터 권하지 않습니다." };
  assert.equal(쓴사람(의사, CONFIG), "병원");
  assert.equal(쓴사람(원장, CONFIG), "원장");
});
...
// 2026-09-15: 정보 유형도 0~12%로 두었더니 93% ~니다 안내문이 됐다 — 이제 둘 다 하한이 있고, 원장이 더 높다.
test("공감 어미가 하나도 없으면 둘 다 권장이 뜨고, 원장의 하한이 더 높다", () => {
  const 원장 = 재기(줄글(짧은줄), { 원장값 });
  const 정보 = 재기(줄글(짧은줄), { 원장값: 정보값 });
  assert.ok(원장.advice.some((s) => s.includes("공감 어미")), 원장.advice.join(" | "));
  assert.ok(정보.advice.some((s) => s.includes("공감 어미")), 정보.advice.join(" | "));
  assert.ok(원장.말투.최소 > 정보.말투.최소);
});
...
// 2026-09-14 우진: "~입니다 위주에 말 거는 문장을 자연스럽게 섞기".
// 말을 거는 어미(~인데요 ~거든요)와 가벼워지는 어미(~어요 ~해요)는 다른 것이다.
test("~인데요 ~거든요는 말을 거는 어미로 세고, ~어요는 아니다", () => {
  const 흐름 = "안녕하세요 원장입니다. 자꾸 올라오시죠? 그 이유는 세안 습관인데요. 해결법을 알려드리겠습니다. " +
    "대부분 제품 탓이라고 오해하십니다. 하지만 제품이 아니라 방치 시간이거든요. 예시를 들어보겠습니다. " +
    "저녁 세안 뒤 20분은 어떤가요? 보시다시피 그 과정이 더 효과적입니다. 다음 회차에 다시 봅니다.";
  const v = 재기(`제목: 여드름 피부관리\n\n${흐름}`, { 원장값 });
  assert.equal(v.말투.공감, 4, "시죠·인데요·거든요·어떤가요 넷");
  assert.ok(!v.advice.some((s) => s.includes("공감 어미")), v.advice.join(" | "));
  assert.ok(!v.advice.some((s) => s.includes("~어요")), v.advice.join(" | "));
});

// 2026-09-15 우진: "~입니다 체가 좀 많은 느낌" — 비율은 기준 안인데 ~입니다가 대여섯 문장씩 줄지어 섰다.
test("~입니다 문장이 연속 허용을 넘겨 이어지면 짚고, 사이에 말 거는 문장이 있으면 조용하다", () => {
  const 줄줄이 = "세안은 저녁에 합니다. 미온수로 헹굽니다. 수건은 누릅니다. 로션은 얇게 바릅니다. 자극은 피합니다. 다음 날 봅니다. ";
  const 끊김 = "세안은 저녁에 합니다. 미온수로 헹굽니다. 자꾸 문지르시죠? 수건은 누릅니다. 로션은 얇게 바릅니다. 그게 이유인데요. 자극은 피합니다. ";
  const a = 재기(`제목: 여드름 피부관리\n\n${줄줄이.repeat(3)}`, { 원장값 });
  const b = 재기(`제목: 여드름 피부관리\n\n${끊김.repeat(3)}`, { 원장값 });
  assert.ok(a.말투.연속단정 >= 6, `연속 ${a.말투.연속단정}`);
  assert.ok(a.advice.some((s) => s.includes("연달아")), a.advice.join(" | "));
  assert.ok(a.말투.연속자리.startsWith("세안은 저녁에"), a.말투.연속자리);
  assert.ok(b.말투.연속단정 <= CONFIG.말투.연속허용, `연속 ${b.말투.연속단정}`);
  assert.ok(!b.advice.some((s) => s.includes("연달아")), b.advice.join(" | "));
  assert.ok(!a.issues.some((s) => s.includes("연달아")), "말투는 불합격 사유가 아니다");
});

// 2026-09-15 두 번째 초안: ~니다는 60%로 내려왔는데 ~거든요가 62문장 중 13번 — 리듬이 버릇이 됐다.
test("말 거는 어미 하나가 상한을 넘겨 되풀이되면 짚는다", () => {
  const 거든요만 = "세안은 저녁에 합니다. 그게 중요하거든요. 수건은 누릅니다. 문지르면 붉어지거든요. 로션은 얇게 바릅니다. 두껍게 바르면 답답하거든요. ";
  const 골고루 = "세안은 저녁에 합니다. 그게 중요하거든요. 수건은 누릅니다. 자꾸 문지르시죠? 로션은 얇게 바릅니다. 두껍게 바르면 어떨까요? ";
  const a = 재기(`제목: 여드름 피부관리\n\n${거든요만.repeat(3)}`, { 원장값 });
  const b = 재기(`제목: 여드름 피부관리\n\n${골고루.repeat(3)}`, { 원장값 });
...
test("지시문의 본보기 문장을 그대로 옮기면 짚는다", () => {
  const v = 재기(구체글() + "\n얼굴을 8곳으로 나눠 순서대로 점검해 드립니다.", { 원장값 });
  assert.ok(v.본보기베낌.includes("얼굴을 8곳으로 나눠"));
  assert.ok(v.advice.some((s) => s.includes("본보기")), v.advice.join(" | "));
  assert.ok(!재기(구체글(), { 원장값 }).advice.some((s) => s.includes("본보기")));
});

test("~어요 문장이 많으면 유형과 상관없이 짚는다", () => {
  const 가벼움 = "세안은 저녁에 한 번만 해요. ";
  const 원장 = 재기(줄글(가벼움), { 원장값 });
  const 정보 = 재기(줄글(가벼움), { 원장값: 정보값 });
  assert.ok(원장.advice.some((s) => s.includes("~어요")), 원장.advice.join(" | "));
  assert.ok(정보.advice.some((s) => s.includes("~어요")), 정보.advice.join(" | "));
  assert.ok(!원장.issues.some((s) => s.includes("~어요")), "말투는 불합격 사유가 아니다");
  const 단정 = 재기(줄글(짧은줄), { 원장값 });
  assert.ok(!단정.advice.some((s) => s.includes("~어요")));
});
...
test("말투·가격·반박은 권장이지 불합격이 아니다 (문장 길이만 2026-09-14부터 불합격)", () => {
  const v = 재기(본문(1400) + " 10회권은 68만원입니다. 회원권은 권하지 않아요.", { 원장값 });
  for (const 말 of ["공감 어미", "약속 문장", "금액 표기", "예약을 막는 생각"])
...
test("검사 스위치는 전부 한 방향이다 — true면 그 검사를 돌린다", () => {
  ...
  assert.ok(공감범위(CONFIG, "원장")[0] > 공감범위(CONFIG, "정보")[0] && 공감범위(CONFIG, "원장")[1] >= 공감범위(CONFIG, "정보")[1], "원장이 더 많이 말을 건다");
});
