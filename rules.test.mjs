...
// ---------- 글쓴이 유형 ----------
// 샵이 있는 원장과, 파는 것 없이 정보만 전하는 사람은 쓰는 방식이 다르다.
// 계정마다 고르고, 원장님들 설정은 건드리지 않는다.
test("두 유형이 서로 다른 화자·구조를 갖는다", () => {
  const 유형 = CONFIG.글쓴이유형;
  assert.ok(유형?.원장 && 유형?.정보, "두 유형이 있어야 한다");
  assert.notEqual(유형.원장.역할, 유형.정보.역할);
  assert.notDeepEqual(유형.원장.화자, 유형.정보.화자);
  assert.notDeepEqual(유형.원장.정보항목, 유형.정보.정보항목);
});

test("정보 전달 유형은 팔지 않는다", () => {
  const 화자 = CONFIG.글쓴이유형.정보.화자.join(" ");
  assert.match(화자, /팔지 않는다|예약·문의·상담 유도를 쓰지 않는다/);
  assert.equal(CONFIG.글쓴이유형.정보.경력검사, false, "샵이 없으면 경력을 펴라고 할 수 없다");
});

test("유형마다 묻는 칸이 갈린다 — 샵 없는 사람에게 관리 구성을 묻지 않는다", () => {
  const 칸 = (t) => (CONFIG.원장정보항목 || []).filter((x) => (x.유형 || "원장") === t).map((x) => x.key);
  assert.deepEqual(칸("원장"), CONFIG.글쓴이유형.원장.정보항목);
  assert.deepEqual(칸("정보"), CONFIG.글쓴이유형.정보.정보항목);
  assert.equal(칸("원장").some((k) => 칸("정보").includes(k)), false, "겹치는 칸이 없어야 한다");
});

test("숫자의 출처가 유형마다 다르다", () => {
  assert.match(CONFIG.글쓴이유형.원장.숫자출처, /관리 시간|가격/);
  assert.match(CONFIG.글쓴이유형.정보.숫자출처, /농도|연구 규모|기간/);
});
