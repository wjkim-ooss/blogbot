...
export function 긴문장(본문들, config) {
  ...
  const 토막들 = config.문장길이?.쉼표끊기
    ? 본문들.flatMap((s) => s.split(/[,，、]\s*/).map((x) => x.trim()).filter(Boolean))
    : 본문들;
  let 합 = 0, 가장긴 = null;
  const 긴것 = [];
  for (const 문장 of 토막들) {
...
export function 말투재기(본문들, config, 범위 = null) {
  const { 공감어미 = [], 약속신호 = [], 부드러운어미 = [], 단정어미 = [], 기본비율 } = config.말투 || {};
  ...
  return {
    ...
    연속단정, 연속자리, 연속허용: config.말투?.연속허용 ?? Infinity,
    // 한 어미만 되풀이되면 리듬이 아니라 버릇이다 — 상한을 넘긴 어미와 횟수
    어미별, 반복어미: Object.entries(어미별).filter(([, n]) => n > (config.말투?.같은어미상한 ?? Infinity)).map(([e, n]) => ({ 어미: e, 횟수: n })),
  };
}
...
const 문구 = {
  지시: {
    ...
    문장길이: (v) =>
      ...
      `${v.문장허용}% 이하로 줄여라 — 한 문장은 ${v.문장하한}~${v.문장상한}자다. 넘는 문장은 마침표나 쉼표로 끊거나 줄을 바꿔 두 줄로 써라. ` +
    ...
    어미반복: (v) =>
      `한 어미가 되풀이된다: ${v.말투.반복어미.map((x) => `~${x.어미} ${x.횟수}번`).join(", ")} — 글 전체에서 ${v.같은어미상한}번을 넘기면 버릇처럼 읽힌다. ` +
    ...
    짧은문장: (v) =>
      `${v.문장하한}자에 못 미치는 토막이 ${v.짧은비율}%다 — 뚝뚝 끊긴다. ${v.문장하한}~${v.문장상한}자 사이에서 짧은 문장과 긴 문장을 섞어라. ` +
  ...
  요약: {
    ...
    문장길이: (v) => `긴 문장 ${v.문장길이.긴것.length}개(${v.문장길이.비율}%) — 한 문장은 공백 빼고 ${v.문장하한}~${v.문장상한}자. 마침표·쉼표로 끊거나 줄을 바꾸세요. 평균 ${v.문장길이.평균}자`,
    말투부족: (v) => `공감 어미 ${v.말투.비율}% — ${v.말투.최소}~${v.말투.최대}% 권장 ("${v.공감본보기}" 같은 문장을 더 섞으세요)`,
    말투과다: (v) => `공감 어미 ${v.말투.비율}%로 많음 — ${v.말투.최대}% 이하 권장 (나머지는 ~입니다로)`,
    어미반복: (v) => `한 어미 되풀이: ${v.말투.반복어미.map((x) => `~${x.어미} ${x.횟수}번`).join(", ")} — ${v.같은어미상한}번 넘으면 버릇처럼 읽힙니다. 다른 어미로 나누세요`,
    ...
    짧은문장: (v) => `${v.문장하한}자 안 되는 토막 ${v.짧은비율}% — 뚝뚝 끊깁니다. 짧은 문장과 긴 문장을 섞으세요`,
...
export function 평가(text, { keyword = "", config, 목표글자수 = 0, ref = null, title, 말투 = "요약", 원장값 = null, 유형 = "", 원장글 = null } = {}) {
  ...
  const 본보기베낌 = (config.본보기문구?.목록 || []).filter((p) => text.includes(p));
  // 문단 끝마다 당부·교훈이 붙으면 훈계문이 된다 — 마지막 줄이 당부 어미로 끝나는 문단을 센다
  const 당부어미 = config.당부?.어미 || [];
  const 당부문단 = 당부어미.length
    ? text.split(/\n\s*\n/).map((p) => p.trim()).filter((p) => p && !/^(#|\[사진|제목:|-\s)/.test(p))
        .map((p) => p.split("\n").at(-1).trim().replace(어미꼬리, ""))
  ...
  // 정보 전달자가 '직접 해본 것'을 안 줬는데 직접 관찰·측정한 것처럼 쓴 문장 — 지어낸 경험이다.
  // 원장값이 없을 때(남의 초안 열람)는 판단할 수 없으니 조용히 넘어간다. 원장 유형은 샵 이야기가 곧 직접 경험이라 보지 않는다.
  const 경험신호 = config.지어낸경험?.신호 || [];
  const 지어낸경험 = 원장값 && 유형정규화(config, 글유형) === "정보" && !(원장값.직접해본것 || "").trim()
    ? 본문만(text, 문장들).filter((s) => 경험신호.some((w) => s.includes(w)))
    : [];
  ...
  // 10~20자라고 전부 10자 안팎으로 뚝뚝 끊으면 그것도 티가 난다 — 하한에 못 미치는 토막이 많으면 짚는다
  const 짧은토막 = 문장길이값.토막들.filter((s) => noSpace(s) < (config.문장길이?.하한 ?? 0));
  const 짧은비율 = 문장길이값.전체 ? Math.round((짧은토막.length / 문장길이값.전체) * 100) : 0;
  ...
  const v = {
    ...
    문장상한: config.문장길이?.상한 ?? 0, 문장하한: config.문장길이?.하한 ?? 0, 문장허용,
    공감본보기: config.말투?.본보기 || "", 약속본보기: config.말투?.약속본보기 || { 나쁨: "", 좋음: "" },
    부드러움허용: config.말투?.부드러움허용 ?? 100, // 없으면 안 걸린다
    첫문단문장수, 가짜PMID, 본보기베낌, 지어낸경험, 당부문단, 짧은비율, 같은어미상한: config.말투?.같은어미상한 ?? Infinity,
  ...
  return {
    ...
    문장길이: 문장길이값, 말투: 말투값, 반박, 가격, 겹침, 약속, 태그, 가짜PMID, 본보기베낌, 지어낸경험, 당부문단, 짧은비율,
