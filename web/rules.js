...
export function 평가(text, { keyword = "", config, 목표글자수 = 0, ref = null, title, 말투 = "요약", 원장값 = null, 유형 = "", 원장글 = null, 사진수 = null } = {}) {
  ...
  return {
    ...
    // 키워드를 안 주면 키워드 검사 넷이 통째로 안 돈다(위에서 전부 `!keyword` 로 빠진다).
    // 안 돌았다는 사실을 판정과 같이 내보낸다 — 검사가 조용히 꺼지면 "통과"가 거짓말이 된다.
    // 발행검증에서 실제로 그랬다: 키워드 없이 돌아 "키워드 0회"로만 찍히고 아무도 몰랐다.
    // 부르는 쪽(편집기·발행검증)은 이 목록을 그대로 보여 주기만 한다.
    꺼진검사: keyword ? [] : ["키워드 횟수", "제목 키워드", "첫 문단 키워드", "소제목 키워드"],
