...
// 이 글을 누구 기준으로 볼 것인가. 목표 글자수와 같은 자리에 같은 방식으로 적어 둔다.
...
export function 적힌유형(content) {
  const 머리 = (content || "").split("\n---\n")[0];
  return (머리.match(/^-\s*글쓴이유형:\s*(\S+)/m) || [])[1] || "";
}
...
export const targetPhotosFor = (ref, config, 목표글자수 = 0) => {
  const 최대 = config.권장이미지최대 ?? 20;
  const 길이몫 = 목표글자수 ? Math.round(목표글자수 / (config.사진간격 || 150)) : 최대;
  return Math.max(config.권장이미지최소, Math.min(ref?.avgImages || 0, 최대, 길이몫));
};
...
export const 유형검사 = (config, 유형) => config.글쓴이유형?.[유형]?.검사 || {};
export const 검사켜짐 = (검사, 키) => 검사?.[키] !== false;
export const 공감범위 = (config, 유형) => config.글쓴이유형?.[유형]?.공감비율 || config.말투?.기본비율 || [0, 100];
...
//
// 조각 크기와 문턱은 보관함 상위글 1,584쌍을 실제로 재서 정했다(config.유사문서.잰것).
const 겹침정규화 = (t) => stripPhotos(t).replace(/[^\p{L}\p{N}\s]/gu, " ").replace(/\s+/g, " ").trim();
const 어절나누기 = (t) => 겹침정규화(t).split(" ").filter(Boolean);
...
export function 겹침찾기(text, ref, config) {
  ...
  if (!설정 || !ref?.posts?.length) return { 겹침률: 0, 최장: 0, 토막: [] };
  const n = 설정.조각어절 || 5;
  const 지도 = 조각지도(ref, n);
  const 어절 = 어절나누기(text);
  const 토막 = [];
  let 맞은수 = 0, 전체 = 0, 시작 = -1, 출처 = "";
  // 이어진 겹침은 길이와 무관하게 다 적어 둔다. 짧은 겹침이 여기저기 흩어져
  // 겹침률만 오르는 글도 있는데, 문턱 넘는 것만 적으면 짚어 줄 대목이 없어 조용해진다.
  const 닫기 = (끝) => {
    if (시작 >= 0) 토막.push({ 말: 어절.slice(시작, 끝).join(" "), 어절수: 끝 - 시작, 출처 });
    시작 = -1;
  };
  for (let i = 0; i + n <= 어절.length; i++) {
    전체 += 1;
    const 나온글 = 지도.get(어절.slice(i, i + n).join(" "));
    if (나온글 === undefined) 닫기(i + n - 1);
    else {
      맞은수 += 1;
      if (시작 < 0) { 시작 = i; 출처 = 나온글; }
    }
  ...
  닫기(어절.length);
  토막.sort((a, b) => b.어절수 - a.어절수);
  return {
    겹침률: 전체 ? Number(((맞은수 / 전체) * 100).toFixed(1)) : 0,
    최장: 토막[0]?.어절수 || 0,
    토막: 토막.slice(0, 설정.보여줄개수 || 3),
  };
}
...
const 문구 = {
  지시: {
    ...
    사진: (v) => `사진 자리 ${v.photos}곳 → ${v.targetPhotos}곳쯤 넣어라 (${v.사진범위})`,
  ...
  요약: {
    ...
    사진: (v) => `사진 자리 ${v.photos}곳 → ${v.targetPhotos}곳쯤이면 좋습니다 (${v.사진범위})`,
...
export function 평가(text, { keyword = "", config, 목표글자수 = 0, ref = null, title, 말투 = "요약", 원장값 = null, 유형 = "" } = {}) {
  ...
  const v = {
    ...
    사진범위: `${config.권장이미지최소}~${config.권장이미지최대 ?? config.권장이미지최소}곳`,
  ...
  if (겹침.최장 >= (config.유사문서?.연속불합격 ?? Infinity)) issues.push(말.베낀문장(v));
  ...
  if (겹침.토막.length && 겹침.최장 < (유사.연속불합격 ?? Infinity) &&
      (겹침.최장 >= (유사.연속경고 ?? Infinity) || 겹침.겹침률 >= (유사.겹침률경고 ?? Infinity)))
    advice.push(말.겹침주의(v));
