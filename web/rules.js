...
export function 평가(text, { keyword = "", config, 목표글자수 = 0, ref = null, title, 말투 = "요약", 원장값 = null, 유형 = "", 원장글 = null, 사진수 = null } = {}) {
  // 재는 덩어리를 여기서 한 모양("제목: X" + 본문)으로 굳힌다.
  // 같은 글이 두 모양으로 들어오고 있었다: 서버·발행검증은 `제목\n본문`, 편집기는 초안 파일
  // 그대로 "제목: …" 줄째. 그래서 판정이 두 군데서 갈렸다 —
  //   ① 글자수: 편집기만 "제목:" 석 자를 더 셌다(3자 차이). 초안 머리에 적힌 글자수와 옆 화면이 달랐다.
  //   ② 문장 검사: 서버는 제목 줄을 본문 문장으로 세었다(본문만()이 "제목:"으로 시작하는 줄만 뺀다).
  //      제목은 길 수밖에 없으니 '긴 문장' 비율이 부풀고, 첫 문단 키워드 검사는 제목을 보고 통과했다.
  ...
  // 글자수는 제목까지 센다 — 다만 "제목:"이라는 표는 글이 아니므로 뺀다.
  ...
  // 위의 넷은 키워드가 없으면 `!keyword` 로 통째로 빠진다. 안 돌았다는 사실을 판정과 같이 내보낸다 —
  // 검사가 조용히 꺼지면 "통과"가 거짓말이 된다. 발행검증에서 실제로 그랬다: 키워드 없이 돌아
  // "키워드 0회"로만 찍히고 아무도 몰랐다. 부르는 쪽(편집기·발행검증)은 그대로 보여 주기만 한다.
  // 이름은 꺼지는 자리 바로 밑에 둔다 — 멀리 두면 키워드 검사를 하나 더 붙일 때 이름만 빠진다.
  const 꺼진검사 = keyword ? [] : ["키워드 횟수", "제목 키워드", "첫 문단 키워드", "소제목 키워드"];
