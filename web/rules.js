...
const 본문문장 = (s) => !/^(#|제목:|제목후보:|\[사진:|-\s*목표글자수|근거\s*[:：])/.test(s);
const 본문만 = (text, 문장들) => (문장들 || 문장나누기(text)).filter(본문문장);

// ---------- 문장 길이 ----------
// 모바일에서 읽힌다는 전제로만 잰다. 목표는 15자지만 억지로 끊으면 어색해지므로
// 불합격이 아니라 권장이다 — 상한을 넘는 문장이 몇 %인지로 본다.
export function 긴문장(text, config, 문장들 = null) {
  const 상한 = config.문장길이?.상한 || 0;
  const 잰것 = 본문만(text, 문장들)
    .map((s) => ({ 문장: s, 길이: noSpace(stripPhotos(s)) }))
    .filter((x) => x.길이);
  const 긴것 = 잰것.filter((x) => 상한 && x.길이 > 상한).sort((a, b) => b.길이 - a.길이);
  return {
    전체: 잰것.length,
    긴것,
    비율: 잰것.length ? Math.round((긴것.length / 잰것.length) * 100) : 0,
    평균: 잰것.length ? Math.round(잰것.reduce((n, x) => n + x.길이, 0) / 잰것.length) : 0,
  };
}
...
export function 말투재기(text, config, 문장들 = null, 범위 = null) {
  const 설정 = config.말투 || {};
  const 문장 = 본문만(text, 문장들);
  // 어미로 판정하므로 문장 끝의 마침표·물음표·따옴표는 걷어내고 본다
  const 끝말 = (s) => s.replace(/[\s"'”’)\]~.!?…]+$/, "");
  const 공감문장 = 문장.filter((s) => (설정.공감어미 || []).some((e) => 끝말(s).endsWith(e)));
  const 약속흐림 = 문장.filter(
    (s) => (설정.약속신호 || []).some((w) => s.includes(w)) && (설정.부드러운어미 || []).some((e) => 끝말(s).endsWith(e))
  );
  const [최소, 최대] = 범위 || 설정.기본비율 || [0, 100];
  const 비율 = 문장.length ? Math.round((공감문장.length / 문장.length) * 100) : 0;
  return { 전체: 문장.length, 공감: 공감문장.length, 비율, 최소, 최대, 약속흐림 };
}
...
export function 반박제거찾기(text, config) {
  const 설정 = config.반박제거;
  if (!설정) return { 짚은것: [], 최소: 0 };
  return { 짚은것: (설정.신호 || []).filter((w) => (text || "").includes(w)), 최소: 설정.최소 ?? 0 };
}
...
export function 가격찾기(text, config) {
  const 꼴 = config.가격표기?.금액;
  if (!꼴) return [];
  return [...new Set(((text || "").match(new RegExp(꼴, "g")) || []).map((s) => s.trim()))];
}
...
const 문구 = {
  지시: {
    ...
    문장길이: (v) =>
      `문장이 길다: ${v.문장상한}자 넘는 문장이 ${v.긴문장수}개(${v.긴문장비율}%, 평균 ${v.문장평균}자). ` +
      `${v.문장허용}% 이하로 줄여라 — 접속사로 이어 붙인 자리를 마침표로 끊어라. 예: "${자르기(v.긴문장예[0], 60)}"`,
    말투비율: (v) =>
      v.공감비율 < v.공감최소
        ? `공감 어미가 ${v.공감비율}%뿐(${v.공감수}/${v.문장수}) — 독자가 겪는 장면 뒤에 "${v.공감본보기}" 같은 문장을 섞어 ${v.공감최소}% 근처로 올려라`
        : `공감 어미가 ${v.공감비율}%로 많다(${v.공감수}/${v.문장수}) — ${v.공감최대}% 이하로 줄이고 나머지는 ~입니다 / ~합니다로 단정해라`,
    약속어미: (v) =>
      `약속하는 문장이 부드러운 어미로 흐른다: ${v.약속흐림.map((s) => `"${자르기(s, 40)}"`).join(", ")} — ` +
      `원장이 책임지는 문장은 반드시 ~합니다로 끝내라 ("권하지 않아요" → "권하지 않습니다")`,
    반박제거: (v) =>
      `예약을 막는 생각을 지운 문장이 없다 — 다음 중 최소 ${v.반박최소}개를 본문 흐름 안에 한 줄로 심어라: ${v.반박걱정.join(" / ")}. ` +
      `Q&A로 따로 빼지 말고, 관리 내용을 보여준 직후에 넣어라. 확실하지 않으면 "[원장확인: 회원권 권유 여부]"로 비워 둬라.`,
  ...
  요약: {
    ...
    문장길이: (v) => `긴 문장 ${v.긴문장수}개(${v.긴문장비율}%) — ${v.문장상한}자 넘으면 폰에서 세 줄로 넘어갑니다. 평균 ${v.문장평균}자`,
    말투비율: (v) =>
      v.공감비율 < v.공감최소
        ? `공감 어미 ${v.공감비율}% — ${v.공감최소}~${v.공감최대}% 권장 ("${v.공감본보기}" 같은 문장을 더 섞으세요)`
        : `공감 어미 ${v.공감비율}%로 많음 — ${v.공감최대}% 이하 권장 (나머지는 ~입니다로)`,
    약속어미: (v) => `약속 문장이 부드럽게 흐릅니다 ${v.약속흐림.length}곳: ${v.약속흐림.map((s) => `"${자르기(s, 30)}"`).join(", ")} — ~합니다로`,
...
export function 평가(text, { keyword = "", config, 목표글자수 = 0, ref = null, title, 말투 = "요약", 원장값 = null } = {}) {
  ...
  const 문장길이값 = 긴문장(text, config, 문장들);
  const 말투값 = 말투재기(text, config, 문장들, 검사설정.공감비율);
  // 반박제거·가격은 파는 사람에게만 해당한다 — 정보형은 예약을 받지도, 가격을 숨길 것도 없다
  const 반박 = 검사설정.반박제거 !== false ? 반박제거찾기(text, config) : { 짚은것: [], 최소: 0 };
  const 가격 = 검사설정.가격표기 !== true ? 가격찾기(text, config) : [];
  ...
  const v = {
    ...
    긴문장수: 문장길이값.긴것.length, 긴문장비율: 문장길이값.비율, 긴문장예: 문장길이값.긴것.map((x) => x.문장),
    문장평균: 문장길이값.평균, 문장상한: config.문장길이?.상한 ?? 0, 문장허용: config.문장길이?.허용비율 ?? 0,
    문장수: 말투값.전체, 공감수: 말투값.공감, 공감비율: 말투값.비율, 공감최소: 말투값.최소, 공감최대: 말투값.최대,
    공감본보기: config.말투?.본보기 || "", 약속흐림: 말투값.약속흐림,
    반박짚음: 반박.짚은것, 반박최소: 반박.최소, 반박걱정: config.반박제거?.걱정 || [],
    가격, 가격대신: config.가격표기?.대신 || "",
  };
  const 말 = 문구[말투];
  ...
  // 아래 셋은 '비율'이라 문장이 몇 개 없으면 숫자가 튄다 — 글다운 길이가 될 때부터 본다
  if (문장길이값.전체 >= 10) {
    if (반박.최소 && 반박.짚은것.length < 반박.최소) advice.push(말.반박제거(v));
    if (문장길이값.비율 > (config.문장길이?.허용비율 ?? 100)) advice.push(말.문장길이(v));
    if (말투값.비율 < 말투값.최소 || 말투값.비율 > 말투값.최대) advice.push(말.말투비율(v));
  }
  ...
  return {
    ...
    문장길이: 문장길이값, 말투: 말투값, 반박: 반박.짚은것, 가격,
