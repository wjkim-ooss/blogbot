...
[문장 규칙 — 폰으로 읽는다는 전제]
- 한 문장은 ${CONFIG.문장길이?.목표}자 안팎을 목표로 한다. 꼭 맞추지 않아도 되지만, 긴 문장이 눈에 띄면 안 된다.
  공백 빼고 ${CONFIG.문장길이?.상한}자를 넘으면 폰에서 세 줄로 넘어간다 — 그런 문장은 두 문장으로 나눠라. 짧은 문장 사이에 긴 문장이 하나 서 있으면 바로 티가 난다.
  내용이 길어져도 문장을 나누는 쪽을 택한다. 나누면 어색한 문장은 줄을 바꿔 여러 줄에 걸쳐 써라 — 한 줄은 ${CONFIG.문장길이?.상한}자 안. 독자는 문장이 아니라 줄을 읽는다.
...
function 말투지시(v) {
  ...
  return [
    ...
    v.말투.반복어미?.length
      ...
      : "",
    `- 말을 걸려고 두 문장을 ~인데요로 잇지 마라. 공백 빼고 ${CONFIG.문장길이?.상한 ?? 40}자를 넘는 문장은 두 문장으로 나눈다 — 폰에서 세 줄로 넘어간다. 나누면 어색한 문장은 줄을 바꿔 여러 줄로.`,
