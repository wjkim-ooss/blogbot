...
// 사람에게 보여 주는 길이·어미 문구는 설정에서 한 번만 읽는다 (rules.js가 재는 값과 같은 자리)
const 문장범위 = 문장길이범위(CONFIG);
const { 하한: 문장하한, 상한: 문장상한 } = CONFIG.문장길이;
const 같은어미상한 = CONFIG.말투.같은어미상한;
...
// 말투 흐름 본보기. 누구에게 줄지는 config.글쓴이유형.*.말투본보기가 정한다 —
// 2026-09-15에 정보 전달자에게도 켰다(담백하게 두었더니 93%가 ~니다인 안내문이 나왔다).
...
[문장 규칙 — 폰으로 읽는다는 전제]
- 한 문장은 공백 빼고 ${문장범위}다. 이 길이를 지킨다. 긴 문장이 눈에 띄면 안 된다.
  ${문장상한}자를 넘을 것 같으면 마침표나 쉼표로 끊어라. 끊기 어색한 문장은 줄을 바꿔 두 줄로 써라 — 한 줄은 ${문장상한}자 안.
  내용이 길어져도 문장을 나누는 쪽을 택한다. 독자는 문장이 아니라 줄을 읽는다. 짧은 줄 사이에 긴 줄이 하나 서 있으면 바로 티가 난다.
  다만 전부 ${문장하한}자 안팎으로 뚝뚝 끊지도 마라 — ${문장하한}자와 ${문장상한}자를 섞고, 한 문장을 두 줄로 잇기도 해라. 5줄씩 같은 모양의 문단을 되풀이하면 그것도 티가 난다.
...
- 말을 거는 어미 하나를 되풀이하지 마라 — ~시죠? ~인데요 ~거든요 ~까요?를 번갈아 쓴다. 한 어미가 글 전체에서 ${같은어미상한}번을 넘으면 버릇처럼 읽힌다.
