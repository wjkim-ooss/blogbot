...
// 조사 붙이기 — "원장이 / 글쓴이가". 받침 유무로 고른다 ("글쓴이이"라고 나가던 것을 고쳤다).
const 이가 = (말) => { const c = 말.charCodeAt(말.length - 1); return 말 + (c >= 0xac00 && c <= 0xd7a3 && (c - 0xac00) % 28 ? "이" : "가"); };
// 말투 흐름 본보기 — 원장에게만. 정보 전달자는 담백해야 하므로 "안녕하세요 ○○입니다"식 리듬을 주지 않는다.
// 같은 내용을 딱딱하게 쓴 것과 말 걸듯 쓴 것 — 실제 초안에서 나온 문장이다. ○○ 본보기보다 이게 더 잘 먹힌다.
const 말투고침 = () => {
  const G = CONFIG.말투?.고침본보기;
  if (!G?.나쁨 || !G?.좋음) return "";
  return `- 같은 내용, 다른 말투 (말투만 보는 본보기다 — 내용을 글에 가져오지 마라):\n  (딱딱함) ${G.나쁨}\n  (이렇게) ${G.좋음}\n`;
};
const 말투흐름 = (유형) => {
  const B = CONFIG.말투?.흐름본보기;
  if (!유형정보(유형).말투본보기 || !B?.줄?.length) return "";
  return `${B.설명}\n${B.줄.map((x) => `  ${x}`).join("\n")}\n`;
};
...
// 사진은 장수를 채우는 일이 아니다. 원장이 실제로 찍어 둘 수 있는 자리만 지목한다 —
// 숫자만 주면 AI가 만든 그림으로 칸을 채우게 되고, 고객은 그걸 알아본다.
const 사진지시 = () => {
  const P = CONFIG.사진자리 || {};
  return [
    `  ${P.설명 || ""}`,
...
[한 글에는 고민 하나 — 이것부터 정하고 시작한다]
쓰기 전에 두 문장을 먼저 정한다. 글 전체가 이 둘에서 벗어나면 안 된다.
  · 고객의 질문 — 이 글에서 고객이 알고 싶은 것 하나
  · 원장의 답  — 이 글이 끝까지 증명할 문장 하나
- 다른 피부 고민을 끌어오지 마라. 모낭염 글에 여드름·좁쌀·홍조를 섞지 않는다.
- 문단마다 "이 문단이 제목의 질문에 답하는가"를 묻고, 답하지 않으면 지운다.
- 제목에 "3가지"라고 썼으면 본문도 정확히 세 항목이고, 셋 다 그 질문에 직접 답해야 한다.
  (검증기가 제목의 숫자와 본문 소제목 수를 실제로 센다)
...
[문장 규칙 — 폰으로 읽는다는 전제]
- 한 문장은 공백 빼고 ${CONFIG.문장길이?.하한}~${CONFIG.문장길이?.상한}자다. 이 길이를 지킨다. 긴 문장이 눈에 띄면 안 된다.
  ${CONFIG.문장길이?.상한}자를 넘을 것 같으면 마침표나 쉼표로 끊어라. 끊기 어색한 문장은 줄을 바꿔 두 줄로 써라 — 한 줄은 ${CONFIG.문장길이?.상한}자 안.
  내용이 길어져도 문장을 나누는 쪽을 택한다. 독자는 문장이 아니라 줄을 읽는다. 짧은 줄 사이에 긴 줄이 하나 서 있으면 바로 티가 난다.
  다만 전부 ${CONFIG.문장길이?.하한}자 안팎으로 뚝뚝 끊지도 마라 — ${CONFIG.문장길이?.하한}자와 ${CONFIG.문장길이?.상한}자를 섞고, 한 문장을 두 줄로 잇기도 해라. 5줄씩 같은 모양의 문단을 되풀이하면 그것도 티가 난다.
- 문단 끝마다 당부·교훈("돈을 아껴야 합니다", "확인하세요")을 붙이지 마라. 당부는 글 끝에 한 번이면 된다. 문단은 사실·장면·숫자로 끝내도 된다. 키워드도 자리를 정해 놓고 박지 말고 문장 흐름에 놓아라.
  소제목은 검색어가 들어가야 하므로 길어도 된다.
- 한 문장에 메시지 하나. "하지만" "그래서"처럼 문장 머리에서 잇는 것은 괜찮다. 한 문장 안에서 -고, -며, -는데로 끌고 가지 마라 — 거기가 문장이 길어지는 자리다.
- 순서·목록은 화살표로 세로 배치한다. 가로로 이으면 폰에서 줄이 어색하게 끊긴다.

[말투 — 상담실에서 고객 앞에 앉아 말하듯]
기본은 ~입니다 / ~합니다다. 그 사이에 말을 거는 문장을 자연스럽게 섞는다.
비율을 맞추는 게 아니라, 설명하다가 고객 쪽을 한 번 보는 리듬이다.
${말투흐름(유형)}${말투고침()}- 말을 거는 자리: 독자가 겪는 장면 다음(~시죠?), 이유를 꺼낼 때(~인데요), 오해를 바로잡을 때(~거든요), 예를 보여줄 때(~어떤가요?).
  대체로 전체 문장의 ${공감최소}~${공감최대}% 안에서 흘러간다. 너무 적으면 안내문이 되고, 그보다 많으면 설명이 사라진다.
- 설명·근거는 ~입니다로 단정한다. 약속하는 문장은 반드시 ~합니다로 끝낸다 — ${이가(부름(유형))} 책임지는 문장이 부드러운 어미로 흐르면 약속이 가벼워진다.
  말을 거는 문장 사이에 단정한 문장이 하나 서 있으면 그 문장만 도드라져서 더 잘 박힌다.
- ~어요 / ~해요 / ~네요로 문장을 끝내지 마라. 가벼워진다. 말을 걸 때도 ~시죠? ~인데요 ~거든요 ~인가요?까지다.
- 같은 어미가 세 번 이어지면 그 자리에 말을 거는 문장을 하나 넣어 끊는다.
- 말을 거는 어미 하나를 되풀이하지 마라 — ~시죠? ~인데요 ~거든요 ~까요?를 번갈아 쓴다. 한 어미가 글 전체에서 ${CONFIG.말투?.같은어미상한 ?? 4}번을 넘으면 버릇처럼 읽힌다.
- 이 지시문의 본보기·예시 문장은 리듬만 보는 것이다. 문장을 그대로 옮기지 마라 — 검증기가 잡는다.
- 어미를 바꾸려고 문장을 비틀지 마라. 어색해질 바에는 ~입니다로 두는 편이 낫다.
...
${B.설명}
${(B.목록 || []).map((x) => `- ${x}`).join("\n")}
이 중 이번 주제와 직접 이어지는 2~3개만 고른다. 분석 과정을 글에 쓰지 말고 문장으로만 드러낸다.
자리는 도입에 하나, 본문 중간에 하나, 결과 직전에 하나. 관계없는 불안을 억지로 끌어오지 마라.
`)}${유형블록(U, "반박제거", (R) => `
[예약을 막는 생각 지우기 — 반박제거]
${R.설명}
...
본보기: "${M.본보기}"
${M.금지}
글 맨 끝에 해시태그 ${M.해시태그.개수[0]}~${M.해시태그.개수[1]}개를 붙인다.
${M.해시태그.규칙.map((x) => `- ${x}`).join("\n")}
`)}
[틀은 같게, 표현은 매번 다르게]
${CONFIG.스타일변주.설명}
${CONFIG.스타일변주.바꿀것.map((x) => `- ${x}`).join("\n")}
${CONFIG.스타일변주.금지}
...
- 고객이 모를 말은 처음 나온 자리에서 1~3문장으로 풀어 쓴다. 사전식 정의 말고 거울이나 사진으로
  알아볼 수 있는 모양과 행동으로 설명하고, 곧바로 지금 사례로 돌아온다.
  예) "농포란 피부가 작게 솟고 가운데 하얗거나 노란 고름이 눈에 보이는 돌기입니다."
- 같은 종결어미를 세 번 연속 쓰지 않는다. 명사형으로 끝나는 문장을 잇달아 쓰지 않는다.
- 질문형 소제목을 쓸 때는 바로 다음 문장에서 답한다. 답을 뒤로 미루지 마라.

[네이버 노출 형식 — 글에 티 나지 않게]
${CONFIG.네이버형식?.설명 ? CONFIG.네이버형식.설명.split("(우진")[0].trim() : ""}
${(CONFIG.네이버형식?.지시 || []).map((x) => `- ${x}`).join("\n")}
...
function buildUserPrompt(keyword, region, point, ref, 목표글자수, shop = null, 사례 = "", 원장본보기 = []) {
  ...
  if (사례) lines.push("", `[이번 글에 쓸 실제 사례 — ${나}가 준 것]`, 사례, "이 사례의 숫자만 쓰고 살을 붙여 지어내지 말 것.");
    ...
    `  └ "직접 측정해 보았습니다", "5종을 직접 구매해 비교했습니다"처럼 실험·측정·구매 비교를 한 것처럼 쓰는 것도 사례를 지어내는 것이다. ${나}가 준 것이 없으면 자료에서 확인한 값만 쓰고, 그것도 없으면 그 대목을 빼라.`);
  ...
  lines.push(
    ...
    `10. 한 문장(한 줄)이 공백 빼고 ${CONFIG.문장길이?.하한 ?? 10}~${CONFIG.문장길이?.상한 ?? 20}자인가 — 넘는 문장은 마침표·쉼표로 끊거나 줄을 바꿔 두 줄로 (기계가 줄과 쉼표 토막마다 잽니다)`,
...
function fixInstruction(v, keyword) {
  ...
  for (const issue of v.issues) {
    if (issue.startsWith("글자수 부족")) {
      ...
      // "모낭염 은"처럼 띄어쓰기로 쪼개 세기를 피하려 든 적이 있다 — 세는 데는 소용없고 글만 이상해진다
      todo.push(`키워드: "${keyword}"가 ${v.kwCount}회로 많다. ${CONFIG.키워드횟수.max}회 이하로 줄이고 나머지는 지시어·유의어로 바꿔라. 띄어쓰기로 쪼개지 마라 — 붙여 세므로 소용없고 글만 이상해진다.`);
...
function 말투지시(v) {
  ...
  return [
    ...
    v.말투.반복어미?.length
      ? `- 한 어미가 되풀이된다: ${v.말투.반복어미.map((x) => `~${x.어미} ${x.횟수}번`).join(", ")}. 글 전체에서 ${CONFIG.말투?.같은어미상한 ?? 4}번까지만 — 넘친 것은 다른 어미로 나누거나 ~입니다로 돌려라.`
      : "",
    `- 말을 걸려고 두 문장을 ~인데요로 잇지 마라. 한 문장은 공백 빼고 ${CONFIG.문장길이?.하한 ?? 10}~${CONFIG.문장길이?.상한 ?? 20}자 — 넘는 문장은 마침표나 쉼표로 끊거나 줄을 바꿔 두 줄로.`,
